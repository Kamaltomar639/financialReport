...
var BlankonTable = function () {
    ...
    return {
        ...
        datatable: function () {
            ...
            var responsiveHelperDomPastEvent = undefined;
            ...
            var tableDompastEvent = $('#datatable-dompastevents');

            // Using AJAX
            tableAjax.dataTable({
                autoWidth: false,
                ajax: "http://localhost/basic/dashboard/usersdata",
            ...
            // Using DOM
            // Remove arrow datatable
            $.extend(true, $.fn.dataTable.default, {
                "aoColumnDefs": [{"bSortable": false, "aTargets": [0, 1, 2, 5]}]
            });
            tableDompastEvent.dataTable({
                autoWidth: false,
                preDrawCallback: function () {
                    // Initialize the responsive datatables helper once.
                    if (!responsiveHelperDomPastEvent) {
                        responsiveHelperDomPastEvent = new ResponsiveDatatablesHelper(tableDompastEvent, breakpointDefinition);
                    }
                },
                rowCallback: function (nRow) {
                    responsiveHelperDomPastEvent.createExpandIcon(nRow);
                },
                drawCallback: function (oSettings) {
                    responsiveHelperDomPastEvent.respond();
                }
            });
